...
import at.bestsolution.tasktracker.emapping.ChangeSetMapper;
...
public final class ChangeSetMapperFactory implements ObjectMapperFactory<at.bestsolution.tasktracker.emapping.ChangeSetMapper,ChangeSet> {
	@Override
	public Class<ChangeSet> getEntityType() {
		return ChangeSet.class;
	}
	
	@Override
	public Class<at.bestsolution.tasktracker.emapping.ChangeSetMapper> getMapperType() {
		return at.bestsolution.tasktracker.emapping.ChangeSetMapper.class;
	}
	
	@Override
	public at.bestsolution.tasktracker.emapping.ChangeSetMapper createMapper(JavaSession session) {
		return new ChangeSetMapperImpl(session);
	}

	final static class ChangeSetMapperImpl implements ChangeSetMapper, at.bestsolution.persistence.java.JavaObjectMapper<ChangeSet>,at.bestsolution.persistence.java.RefreshableObjectMapper<ChangeSet>, ResolveDelegate {
		...
		public long selectVersion(Object id) {
			...
			try {
				try {
					pStmt = connection.prepareStatement("SELECT " + getLockColumn() + " FROM \"change_set\" WHERE cs_id = ?");
		...
		// by JavaObjectMapperGenerator
		@Override
		public final at.bestsolution.persistence.MappedUpdateQuery<ChangeSet> deleteAllMappedQuery() {
			MappedUpdateQuery<ChangeSet> deleteQuery = session.getDatabaseSupport().createMappedUpdateQuery(this, null,
				new UpdateDelegate<ChangeSet>() { public int execute(MappedUpdateQuery<ChangeSet> criteria) { return deleteAll((InternalQueryCriteria)criteria); } }
			);
			return deleteQuery;
		}
		...
		public final ChangeSet map_default_ChangeSet(Connection connection, ResultSet set) throws SQLException {
			Long id = set.getLong("cs_id");
			final EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet();
			...
			rv.setSid(set.getLong("cs_id"));
			rv.setComment(set.getString("cs_comment"));
			rv.setId(set.getString("cs_vcs_id"));
			ProxyData_ChangeSet proxy = new ProxyData_ChangeSet(set.getLong("cs_fk_person"),set.getLong("cs_fk_repository"),set.getLong("cs_fk_task"));
		...
		private final void map_default_ChangeSet_data_refresh(ChangeSet rv, Connection connection, ResultSet set) throws SQLException {
			rv.setSid(set.getLong("cs_id"));
			rv.setComment(set.getString("cs_comment"));
			rv.setId(set.getString("cs_vcs_id"));
		}

		private final void map_default_ChangeSet_complete_refresh(ChangeSet rv, Connection connection, ResultSet set, Set<Object> refreshedObjects) throws SQLException {
			rv.setSid(set.getLong("cs_id"));
			rv.setComment(set.getString("cs_comment"));
			rv.setId(set.getString("cs_vcs_id"));
			ProxyData_ChangeSet proxy = new ProxyData_ChangeSet(set.getLong("cs_fk_person"),set.getLong("cs_fk_repository"),set.getLong("cs_fk_task"));
			...
			{
				EObject eo = (EObject)rv;
				EReference r = (EReference)eo.eClass().getEStructuralFeature("sourceRepository");
				if( ((LazyEObject)rv).isResolved(r) ) {
					at.bestsolution.tasktracker.model.tasktracker.SourceRepository v = rv.getSourceRepository();
					RefreshableObjectMapper<at.bestsolution.tasktracker.model.tasktracker.SourceRepository> mr = (RefreshableObjectMapper<at.bestsolution.tasktracker.model.tasktracker.SourceRepository>)session.createMapper(at.bestsolution.tasktracker.emapping.SourceRepositoryMapper.class);
					if( v != null && ! refreshedObjects.contains(v) ) {
						mr.refreshWithReferences(v,refreshedObjects);
					} else {
						long currentId = v == null ? 0 : ((Number)mr.getPrimaryKeyValue(v)).longValue();
						if( currentId != proxy.sourceRepository ) {
							EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getSourceRepository();
							v = session.getCache().getObject(eClass,proxy.sourceRepository);
							if( v != null ) {
								resolve((LazyEObject)rv,proxy,r);
								if( ! refreshedObjects.contains(v) ) {
									mr.refreshWithReferences(v,refreshedObjects);
			...
			private List<Object> extractObjectIds(ChangeSet... object) {
		...
		// by QueryGenerator
		@Override
		public final java.util.List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> selectAll() {
		...
		// by QueryGenerator
		public final MappedQuery<ChangeSet> selectAllMappedQuery() {
		...
		// by QueryGenerator
		@Override
		public final at.bestsolution.tasktracker.model.tasktracker.ChangeSet selectById(final long sid) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Check id cache for object");
			{
				final EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet();
				final ChangeSet rv = session.getCache().getObject(eClass,sid);
				if( rv != null ) {
		...
		// by QueryGenerator
		@Override
		public final java.util.List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> selectAllForPerson(final long pId) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Executing selectAllForPerson");
		
			String query;
				query = Util.loadFile(getClass(), "ChangeSet_selectAllForPerson_default.sql");
		...
		// by QueryGenerator
		@Override
		public final java.util.List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> selectAllForSourceRepository(final long srId) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) LOGGER.debug("Executing selectAllForSourceRepository");
		
			String query;
				query = Util.loadFile(getClass(), "ChangeSet_selectAllForSourceRepository_default.sql");
			...
			int paramIndex = 1;
			for(int i = 0; i < processedSQL.dynamicParameterNames.size(); i++) {
				if( "srId".equals(processedSQL.dynamicParameterNames.get(i)) ) {
					if( isDebug ) {
						debugParams.add("srId = " + srId);
					}
					pStmt.setLong(paramIndex++,srId);
				}
			}
			if( isDebug ) {
				LOGGER.debug(" Dynamic-Parameters: " + debugParams);
		...
		// by QueryGenerator
		@Override
		public final java.util.List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> selectAllForTask(final long tId) {
			final boolean isDebug = LOGGER.isDebugEnabled();
		...
		public final List<Long> selectAllObjectIds() {
			return session.getDatabaseSupport().createMappedSelectQuery(this, null, new at.bestsolution.persistence.java.query.DynamicListDelegate<Long,ChangeSet>() {
				public List<Long> list(at.bestsolution.persistence.DynamicSelectQuery<Long,ChangeSet> criteria) {
					return selectAllObjectIds( (at.bestsolution.persistence.java.query.DynamicSelectQueryImpl<Long,ChangeSet>)criteria );
				}
			}).list();
		}
		
		public final at.bestsolution.persistence.DynamicSelectQuery<Long,ChangeSet> selectAllObjectIdsMappedQuery() {
			return session.getDatabaseSupport().createMappedSelectQuery(this, null, new at.bestsolution.persistence.java.query.DynamicListDelegate<Long,ChangeSet>() {
				public List<Long> list(at.bestsolution.persistence.DynamicSelectQuery<Long,ChangeSet> criteria) {
					return selectAllObjectIds( (at.bestsolution.persistence.java.query.DynamicSelectQueryImpl<Long,ChangeSet>)criteria );
				}
		...
		public final List<Long> selectAllObjectIds(at.bestsolution.persistence.java.query.DynamicSelectQueryImpl<Long,ChangeSet> criteria) {
			final boolean isDebug = LOGGER.isDebugEnabled();
		
			String query = "SELECT \"cs_id\" FROM \"change_set\"";
		...
		// refresh stuff
		// by QueryGenerator
		public final void refresh(final ChangeSet object, final at.bestsolution.persistence.Session.RefreshType refreshType) {
			switch( refreshType ) {
				case DATA_ONLY:
		...
		public final void refreshWithReferences(final ChangeSet object, Set<Object> refreshedObjects) {
			...
			try {
				...
				if( set.next() ) {
					...
					map_default_ChangeSet_complete_refresh(object,connection,set,refreshedObjects);
		...
		private final void refreshDataOnly(final ChangeSet object, boolean updateVersion) {
			String query;
				query = Util.loadFile(getClass(), "ChangeSet_selectById_default.sql");
			...
			try {
				...
				set = pStmt.executeQuery();
				if( set.next() ) {
					map_default_ChangeSet_data_refresh(object,connection,set);
		...
		// update stuff
		@Override
		public final void update(final ChangeSet object) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) {
			...
			// Built the query
			at.bestsolution.persistence.java.DatabaseSupport.UpdateStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"change_set").createUpdateStatement("cs_id", getLockColumn());
			// NEW:
			// simple direct mapped attributes
			// * comment
			stmt.addString("cs_comment", (java.lang.String)session.getTransactionAttribute(object,at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Comment()));
			// * id
			stmt.addString("cs_vcs_id", (java.lang.String)session.getTransactionAttribute(object,at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Id()));
			...
			if( object.getPerson() != null ) {
				...
				stmt.addLong("cs_fk_person", refKey);
			} else {
				stmt.addNull("cs_fk_person",getJDBCType("person"));
			}
			// * sourceRepository
			if( object.getSourceRepository() != null ) {
				final at.bestsolution.tasktracker.emapping.SourceRepositoryMapper refMapper = session.createMapper(at.bestsolution.tasktracker.emapping.SourceRepositoryMapper.class);
				final long refKey = session.getPrimaryKey(refMapper, object.getSourceRepository());
				stmt.addLong("cs_fk_repository", refKey);
			} else {
				stmt.addNull("cs_fk_repository",getJDBCType("sourceRepository"));
			}
			// * task
			if( object.getTask() != null ) {
				...
				stmt.addLong("cs_fk_task", refKey);
			} else {
				stmt.addNull("cs_fk_task",getJDBCType("task"));
			}
		...
		// insert stuff
		@Override
		public final void insert(final ChangeSet object) {
			final boolean isDebug = LOGGER.isDebugEnabled();
			if( isDebug ) {
				LOGGER.debug("Starting insert of '"+object+"'");
			}
			...
			// Build the SQL
			at.bestsolution.persistence.java.DatabaseSupport.InsertStatement stmt = session.getDatabaseSupport().createQueryBuilder(this,"change_set").createInsertStatement("cs_id", sequenceExpression, getLockColumn());
		
		
			// handle simple direct mapped attributes
			// * comment
			{
				Object o = session.getTransactionAttribute(object,at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Comment());
				if( o != null ) {
					stmt.addString("cs_comment", (java.lang.String)o);
				}
			}
			// * id
			{
				Object o = session.getTransactionAttribute(object,at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Id());
				if( o != null ) {
					stmt.addString("cs_vcs_id", (java.lang.String)o);
				}
			}
			...
			if( object.getPerson() != null ) {
				...
				final long refKey = session.getPrimaryKey(refMapper, object.getPerson());
				stmt.addLong("cs_fk_person", refKey);
				//stmt.addLong("cs_fk_person",object.getPerson().getSid());
			}
			// * sourceRepository
			if( object.getSourceRepository() != null ) {
				final at.bestsolution.tasktracker.emapping.SourceRepositoryMapper refMapper = session.createMapper(at.bestsolution.tasktracker.emapping.SourceRepositoryMapper.class);
				final long refKey = session.getPrimaryKey(refMapper, object.getSourceRepository());
				stmt.addLong("cs_fk_repository", refKey);
				//stmt.addLong("cs_fk_repository",object.getSourceRepository().getSid());
			}
			// * task
			if( object.getTask() != null ) {
				final at.bestsolution.tasktracker.emapping.TaskMapper refMapper = session.createMapper(at.bestsolution.tasktracker.emapping.TaskMapper.class);
				final long refKey = session.getPrimaryKey(refMapper, object.getTask());
				stmt.addLong("cs_fk_task", refKey);
				//stmt.addLong("cs_fk_task",object.getTask().getSid());
			}
		...
		final int deleteAll(InternalQueryCriteria criteria) {
			...
			session.preExecuteDeleteMany(at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet());
		
			// build delete query
			String deleteQuery = "DELETE FROM \"change_set\"";
		...
		public final void deleteAll() {
			...
			session.scheduleAfterTransaction(new at.bestsolution.persistence.java.UnregisterAllObjectsAfterTx(at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet()));
		
			final Connection connection = session.checkoutConnection();
			try {
		
				// find all object ids
				String objectIdSQL = "SELECT \"cs_id\" FROM \"change_set\"";
				...
				try {
					...
					while (objectIdResultSet.next()) {
						objectIds.add(objectIdResultSet.getLong("cs_id"));
					}
				}
				finally {
		...
		public final void deleteById(List<Object> objectIds) {
			...
			try {
				...
				sqlBuilder.append("DELETE FROM \"change_set\" WHERE \"cs_id\" IN (");
		...
		final boolean doResolve(LazyEObject eo, Object proxyData, EStructuralFeature f) {
			...
			if(eo instanceof ChangeSet) {
				at.bestsolution.tasktracker.model.tasktracker.ChangeSet target = (at.bestsolution.tasktracker.model.tasktracker.ChangeSet)eo;
				switch(f.getFeatureID()) {
				  // person
				  case at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.CHANGE_SET__PERSON: {
				    {
				      EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getPerson();
				      at.bestsolution.tasktracker.model.tasktracker.Person o = session.getCache().getObject(eClass, ((ProxyData_ChangeSet)proxyData).person);
				      if( o == null ) {
				        o = session.createMapper(at.bestsolution.tasktracker.emapping.PersonMapper.class).selectById(((ProxyData_ChangeSet)proxyData).person);
				      } else {
				        if( LOGGER.isDebugEnabled() ) {
				          LOGGER.debug("Using cached version");
				        }
				  ...
				  // sourceRepository
				  case at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.CHANGE_SET__SOURCE_REPOSITORY: {
				    {
				      EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getSourceRepository();
				      at.bestsolution.tasktracker.model.tasktracker.SourceRepository o = session.getCache().getObject(eClass, ((ProxyData_ChangeSet)proxyData).sourceRepository);
				      if( o == null ) {
				        o = session.createMapper(at.bestsolution.tasktracker.emapping.SourceRepositoryMapper.class).selectById(((ProxyData_ChangeSet)proxyData).sourceRepository);
				      } else {
				        if( LOGGER.isDebugEnabled() ) {
				          LOGGER.debug("Using cached version");
				        }
				  ...
				  // task
				  case at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.CHANGE_SET__TASK: {
				    {
				      EClass eClass = at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getTask();
				      at.bestsolution.tasktracker.model.tasktracker.Task o = session.getCache().getObject(eClass, ((ProxyData_ChangeSet)proxyData).task);
				      if( o == null ) {
				        o = session.createMapper(at.bestsolution.tasktracker.emapping.TaskMapper.class).selectById(((ProxyData_ChangeSet)proxyData).task);
				      } else {
				        if( LOGGER.isDebugEnabled() ) {
				          LOGGER.debug("Using cached version");
				        }
		...
		static {
			PROPERTY_COL_MAPPING.put("sid","cs_id");
			PROPERTY_COL_MAPPING.put("comment","cs_comment");
			PROPERTY_COL_MAPPING.put("id","cs_vcs_id");
			PROPERTY_COL_MAPPING.put("person","cs_fk_person");
			PROPERTY_COL_MAPPING.put("sourceRepository","cs_fk_repository");
			PROPERTY_COL_MAPPING.put("task","cs_fk_task");
			...
			REF_ID_FEATURES.put("sourceRepository",at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getBaseObject_Sid());
			...
			REFERENCE_MAPPER.put("sourceRepository",at.bestsolution.tasktracker.emapping.SourceRepositoryMapper.class);
			REFERENCE_MAPPER.put("task",at.bestsolution.tasktracker.emapping.TaskMapper.class);
			REFERENCE_FEATURES.add(at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Person());
			REFERENCE_FEATURES.add(at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_SourceRepository());
			REFERENCE_FEATURES.add(at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet_Task());
		}
		
		public EClass getEClass() {
			return at.bestsolution.tasktracker.model.tasktracker.TasktrackerPackage.eINSTANCE.getChangeSet();
		}
		...
		protected final <P> P getPrimaryKeyForTx(ChangeSet o) {
			return session.getPrimaryKey(this, o);
		}

		protected final long getVersionForTx(ChangeSet o) {
			return session.getVersion(this, o);
		}
	}

public final NamedQuery<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> createNamedQuery(final JavaSession session, String name) {
	if( "selectAll".equals(name) ) {
		return new NamedQuery<at.bestsolution.tasktracker.model.tasktracker.ChangeSet>() {
			public final List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> queryForList(Object... parameters) {
				return createMapper(session).selectAll();
			}

			public final at.bestsolution.tasktracker.model.tasktracker.ChangeSet queryForOne(Object... parameters) {
	...
	if( "selectAllForSourceRepository".equals(name) ) {
		return new NamedQuery<at.bestsolution.tasktracker.model.tasktracker.ChangeSet>() {
			public final List<at.bestsolution.tasktracker.model.tasktracker.ChangeSet> queryForList(Object... parameters) {
				return createMapper(session).selectAllForSourceRepository(((Long)parameters[0]).longValue());
			}
