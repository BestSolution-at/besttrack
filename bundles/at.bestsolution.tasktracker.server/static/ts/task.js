...
function handleTaskData(data, err) {
    task = data;
    ...
    $("#bug-form-task-version").each(function () {
        $(this).html('<option value="" disabled selected>Version</option>'
            +
                optionList.join("\n"));
    });
    $("#bug-form-task-fix").each(function () {
        $(this).html('<option value="" disabled selected>Fixed in Version</option>'
            +
                optionList.join("\n"));
    });
    ...
    $("#comment-list").each(function () {
        $(this).html(commentList.join("\n"));
    });
}
function commentToHTML(o) {
...
function updateTask() {
    ...
    var taskService = new TaskService(rootPath);
    taskService.update(task, function (err) {
    });
}
function submitComment() {
    ...
    commentService.create(comment, function (comment, err) {
        $("#comment-list").each(function () {
